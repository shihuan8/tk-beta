...
const speedToMbps = (speed?: number): number => {
  const value = Number(speed) || 0;

  if (value >= BPS_PER_MBPS) {
    return value / BPS_PER_MBPS;
  }

  return value;
};

const speedFromMbps = (speed: number): number => {
  return Math.round(Math.max(0, Number(speed) || 0) * BPS_PER_MBPS);
};

const formatSpeedMbps = (speed?: number): string => {
  const value = speedToMbps(speed);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        id: form.id,
        name: form.name,
        speed: speedFromMbps(form.speed),
        status: form.status,
      };
                ...
                <TableRow key={rule.id}>
                  ...
                  <TableCell>
                    <span className="text-sm font-mono text-default-600">
                      {formatSpeedMbps(rule.speed)} Mbps
                    </span>
                  </TableCell>
                  <TableCell>
