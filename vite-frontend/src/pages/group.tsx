...
export default function GroupPage() {
  ...
  const [tunnelGroupSpeedLimit, setTunnelGroupSpeedLimit] = useState("0");
  ...
  const openEditTunnelGroup = (group: TunnelGroup) => {
    ...
    setTunnelGroupSpeedLimit(String((group.speedLimit || 0) / 1_000_000));
    onTunnelGroupModalOpen();
  };
  ...
  const saveTunnelGroup = async () => {
    ...
    try {
      const payload = {
        ...
        speedLimit: Math.max(0, Number(tunnelGroupSpeedLimit) || 0) * 1_000_000,
      };
            ...
            <TableBody emptyContent="暂无隧道分组" items={tunnelGroups}>
              {(item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell>
                    {item.speedLimit && item.speedLimit > 0
                      ? `${(item.speedLimit / 1_000_000).toFixed(1)} Mbps`
                      : "不限速"}
                  </TableCell>
                  <TableCell>
                    <Chip
        ...
        <ModalContent>
          ...
          <ModalBody className="space-y-3">
            ...
            <Input
              ...
              placeholder="0 表示不限速"
              type="number"
              value={tunnelGroupSpeedLimit}
              onChange={(e) => setTunnelGroupSpeedLimit(e.target.value)}
            />
            <Select
              label="状态"
