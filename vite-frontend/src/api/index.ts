import type {
  ...
  PaymentStatsApiData,
  OBSCodeAssignmentApiItem,
  OrderApiItem,
  SubscriptionPackageApiItem,
  UserQuotaHistoryItem,
} from "./types";
import type { UserPackagePermission } from "../types";
...
export const getUserPackagePermissions = (id: number) =>
  Network.post<UserPackagePermission[]>("/user/package-permissions", { id });
...
export const assignPackageGroupToUserTunnel = (data: {
  userId: number;
  userGroupId: number;
}) => Network.post("/tunnel/user/assign-package-group", data);
...
export const batchDeleteUsers = (ids: number[]) =>
  Network.post<BatchOperationResult>("/user/batch-delete", { ids });
export const batchResetUserFlow = (ids: number[]) =>
  Network.post<BatchOperationResult>("/user/batch-reset", { ids });
...
export const createTunnelGroup = (data: { name: string; status?: number; speedLimit?: number }) =>
...
export const getPlanList = () => Network.post<PlanApiItem[]>("/plan/list");
export const createPlan = (data: PlanPayload) =>
  Network.post<PlanApiItem>("/plan/create", data);
export const updatePlan = (data: PlanPayload) =>
  Network.post<PlanApiItem>("/plan/update", data);
export const deletePlan = (id: number) => Network.post("/plan/delete", { id });
export const getAvailablePlans = () =>
  Network.post<PlanApiItem[]>("/plan/available");
export const getStoreStatus = () =>
  Network.post<{ enabled: boolean }>("/plan/store-status");
export const setStoreStatus = (data: { enabled: boolean }) =>
  Network.post("/plan/store-status/save", data);
export const purchasePlan = (planId: number) =>
  Network.post<UserPlanApiItem>("/plan/purchase", { planId });
export const getUserPlans = () =>
  Network.post<UserPlanApiItem[]>("/user/plan/list");
export const listAutoBuyTrafficPackages = () =>
  Network.post<SubscriptionPackageApiItem[]>("/package/auto-buy-traffic/list");
export const getUserBalance = () =>
  Network.post<UserBalanceApiItem>("/user/balance");
export const setUserBalance = (userId: number, balance: number) =>
  Network.post("/balance/set", { userId, balance });
export const getBalanceList = () =>
  Network.post<BalanceWithUserApiItem[]>("/balance/list");
export const getBalanceLogs = (data: { page?: number; size?: number; userId?: number } = {}) =>
  Network.post<{ list: BalanceLogApiItem[]; total: number }>("/balance/logs", data);
export const deleteBalanceLog = (id: number) =>
  Network.post("/balance/log/delete", { id });
export const cleanupBalanceLogs = () => Network.post<{ deleted: number }>("/balance/log/cleanup");
export const getPaymentConfigs = () =>
  Network.post<PaymentConfigApiItem[]>("/payment/config");
export const getAllPaymentConfigs = () =>
  Network.post<PaymentConfigApiItem[]>("/payment/config/admin/list");
export const savePaymentConfig = (data: { channel: string; config: unknown; enabled: number }) =>
  Network.post("/payment/config/save", data);
export const deletePaymentConfig = (channel: string) =>
  Network.post("/payment/config/delete", { channel });
export const getPaymentStats = () => Network.post<PaymentStatsApiData>("/payment/stats");
export const getAdminOrderList = (data: { page?: number; size?: number; status?: number; keyword?: string; userId?: number } = {}) =>
  Network.post<{ list: OrderApiItem[]; total: number }>("/order/admin/list", data);
export const getUserOrderList = (data: { page?: number; size?: number; status?: number; keyword?: string } = {}) =>
  Network.post<{ list: OrderApiItem[]; total: number }>("/order/list", data);
export const updateOrder = (data: { id: number; status?: number; amount?: number; payTime?: number; payCurrency?: string; productName?: string }) =>
  Network.post("/order/admin/update", data);
export const deleteOrder = (id: number) => Network.post("/order/admin/delete", { id });
export const refundOrder = (id: number) => Network.post("/order/admin/refund", { id });

export const getOBSCodeList = () =>
  Network.post<OBSCodeAssignmentApiItem[]>("/obs-code/list");
export const getMyOBSCode = () =>
  Network.post<OBSCodeAssignmentApiItem | null>("/obs-code/mine");
export const saveOBSCode = (data: {
  userId: number;
  userName: string;
  pushCode: string;
  inputCode: string;
  remark?: string;
}) => Network.post("/obs-code/save", data);
export const deleteOBSCode = (id: number) => Network.post("/obs-code/delete", { id });
