...
func (r *Repository) ResetUserFlowByUser(userID int64, now int64) {
	...
	var user model.User
	if err := r.db.Select("in_flow", "out_flow").Where("id = ?", userID).First(&user).Error; err == nil {
		_ = r.db.Create(&model.UserQuotaHistory{
			UserID:        userID,
			ResetTime:     now,
...
func (r *Repository) GetUserQuotaHistory(userID int64, limit int) ([]model.UserQuotaHistory, error) {
	...
	var items []model.UserQuotaHistory
...
func (r *Repository) UpdateUserOrder(userID int64, _ int, now int64) error {
	if r == nil || r.db == nil || userID <= 0 {
		return nil
	}
	return r.db.Model(&model.User{}).Where("id = ?", userID).Update("updated_time", sql.NullInt64{Int64: now, Valid: true}).Error
}
...
func (r *Repository) GroupCreate(table, name string, status int, now int64) error {
	return r.GroupCreateWithPackage(table, name, status, 0, 0, 0, 0, now)
}

func (r *Repository) GroupCreateWithPackage(table, name string, status int, flowLimit, speedLimit int64, ruleQuota, durationDays int, now int64) error {
	if r == nil || r.db == nil {
		return errors.New("repository not initialized")
	}
	switch table {
...
func (r *Repository) GroupUpdate(table string, id int64, name string, status int, now int64) error {
	return r.GroupUpdateWithPackage(table, id, name, status, 0, 0, 0, 0, now)
}

func (r *Repository) GroupUpdateWithPackage(table string, id int64, name string, status int, flowLimit, speedLimit int64, ruleQuota, durationDays int, now int64) error {
	if r == nil || r.db == nil {
		return errors.New("repository not initialized")
	}
	...
	switch table {
	case "tunnel_group":
		updates["speed_limit"] = speedLimit
		return r.db.Model(&model.TunnelGroup{}).Where("id = ?", id).Updates(updates).Error
	case "user_group":
		updates["flow_limit"] = flowLimit
		updates["speed_limit"] = speedLimit
		updates["rule_quota"] = ruleQuota
