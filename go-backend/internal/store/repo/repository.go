...
type UserBalance = model.UserBalance
type UserPlan = model.UserPlan
...
func (r *Repository) ListUsers() ([]map[string]interface{}, error) {
	...
	balanceByUser := make(map[int64]model.UserBalance, len(userIDs))
	if len(userIDs) > 0 {
	...
	for _, u := range users {
		...
		if balance, ok := balanceByUser[u.ID]; ok {
			item["balance"] = balance.Balance * 100
		} else {
			item["balance"] = 0
		}
...
func (r *Repository) ListUserGroups() ([]map[string]interface{}, error) {
	...
	for _, g := range groups {
		...
		result = append(result, map[string]interface{}{
			"id": g.ID, "name": g.Name, "status": g.Status,
			"flowLimit": g.FlowLimit, "speedLimit": g.SpeedLimit,
			"ruleQuota": g.RuleQuota, "durationDays": g.DurationDays,
...
const storeBytesPerGB int64 = 1024 * 1024 * 1024
...
var migrateOversizedPlanFlowQuotasFn = migrateOversizedPlanFlowQuotas
var migrateRefundedPlanOrdersFn = migrateRefundedPlanOrders
var migrateTunnelGroupSpeedLimitFn = migrateTunnelGroupSpeedLimit
...
func migrateSchema(db *gorm.DB) error {
	...
	if ver < 7 {
		if err := migrateOversizedPlanFlowQuotasFn(db); err != nil {
			return err
		}
	}

	if ver < 8 {
		if err := migrateRefundedPlanOrdersFn(db); err != nil {
			return err
		}
	}

	if ver < 9 {
		if err := migrateTunnelGroupSpeedLimitFn(db); err != nil {
			return err
		}
	}
...
func migrateTunnelGroupSpeedLimit(db *gorm.DB) error {
	if db == nil {
		return errors.New("nil db")
	}
	if !db.Migrator().HasTable(&model.TunnelGroup{}) {
		return nil
	}
	if !db.Migrator().HasColumn(&model.TunnelGroup{}, "speed_limit") {
		if err := db.Migrator().AddColumn(&model.TunnelGroup{}, "SpeedLimit"); err != nil {
			return fmt.Errorf("add tunnel_group.speed_limit: %w", err)
		}
	}
	return nil
}

func migrateRefundedPlanOrders(db *gorm.DB) error {
	if db == nil {
		return errors.New("nil db")
	}
	if !db.Migrator().HasTable(&model.Order{}) || !db.Migrator().HasTable(&model.UserPlan{}) {
		return nil
	}
	if err := db.Exec(`UPDATE user_plan
		SET status = 3
		WHERE status = 1
		  AND EXISTS (
		    SELECT 1 FROM "order" o
		    WHERE o.status = 3
		      AND o.product_type = 'plan'
		      AND o.user_id = user_plan.user_id
		      AND o.product_id = user_plan.plan_id
		  )`).Error; err != nil {
		return fmt.Errorf("expire refunded plan orders: %w", err)
	}
	return nil
}

func migrateOversizedPlanFlowQuotas(db *gorm.DB) error {
	if db == nil {
		return errors.New("nil db")
	}
	...
	for _, item := range []struct {
		...
		if err := db.Table(item.table).
			Where("flow >= ?", storeBytesPerGB).
			UpdateColumn("flow", gorm.Expr("flow / ?", storeBytesPerGB)).Error; err != nil {
			return fmt.Errorf("normalize oversized %s.flow quota: %w", item.table, err)
		}
	}
