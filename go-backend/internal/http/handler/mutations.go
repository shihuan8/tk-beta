...
func isAdminRoleID(roleID int) bool {
	return roleID == rolePrimaryAdmin || roleID == roleSubAdmin
}

func currentRoleIDFromRequest(r *http.Request) (int, error) {
...
func (h *Handler) userCreate(w http.ResponseWriter, r *http.Request) {
	...
	flow := asInt64(req["flow"], 0)
	if flow < 0 {
		response.WriteJSON(w, response.ErrDefault("流量限制不能小于0"))
		return
	}
	num := asInt(req["num"], 0)
	if num < 0 {
		response.WriteJSON(w, response.ErrDefault("规则数量不能小于0"))
		return
	}
	balance, hasBalance := req["balance"]
	if hasBalance && asFloat(balance, 0) < 0 {
		response.WriteJSON(w, response.ErrDefault("可用余额不能小于0"))
		return
	}
	...
	roleID := asInt(req["roleId"], roleUser)
	if roleID != roleUser && roleID != roleSubAdmin {
		roleID = roleUser
	}
	if roleID == roleSubAdmin {
		actorRoleID, err := currentRoleIDFromRequest(r)
		if err != nil {
			response.WriteJSON(w, response.Err(401, "无效的token或token已过期"))
			return
		}
		if actorRoleID != rolePrimaryAdmin {
			response.WriteJSON(w, response.Err(403, "仅主管理员可创建副管理员"))
			return
		}
	}
	...
	if hasBalance {
		if err := h.repo.SetUserBalance(userID, asFloat(balance, 0)/100); err != nil {
			response.WriteJSON(w, response.Err(-2, err.Error()))
			return
		}
	}
...
func (h *Handler) userUpdate(w http.ResponseWriter, r *http.Request) {
	...
	if roleID == rolePrimaryAdmin {
		response.WriteJSON(w, response.ErrDefault("请不要作死"))
		return
	}
	if roleID == roleSubAdmin {
		actorRoleID, actorErr := currentRoleIDFromRequest(r)
		if actorErr != nil {
			response.WriteJSON(w, response.Err(401, "无效的token或token已过期"))
			return
		}
		if actorRoleID != rolePrimaryAdmin {
			response.WriteJSON(w, response.Err(403, "仅主管理员可编辑副管理员"))
			return
		}
	}
	...
	_ = h.repo.RestoreActiveUserPlanTunnels(id)
...
func (h *Handler) userPackagePermissions(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		response.WriteJSON(w, response.ErrDefault("请求失败"))
		return
	}
	...
	permissions, err := h.repo.ListUserPackagePermissions(id)
	if err != nil {
		response.WriteJSON(w, response.Err(-2, err.Error()))
		return
	}
	response.WriteJSON(w, response.OK(permissions))
}
...
func (h *Handler) userDelete(w http.ResponseWriter, r *http.Request) {
	...
	if roleID == roleSubAdmin {
		...
		if actorRoleID != rolePrimaryAdmin {
			response.WriteJSON(w, response.Err(403, "仅主管理员可删除副管理员"))
			return
		}
	}
...
func (h *Handler) userBatchDelete(w http.ResponseWriter, r *http.Request) {
	...
	for _, id := range req.IDs {
		roleID, err := h.repo.GetUserRoleID(id)
		if err != nil || roleID == rolePrimaryAdmin {
			fail++
			continue
		}
		if roleID == roleSubAdmin {
			actorRoleID, actorErr := currentRoleIDFromRequest(r)
			if actorErr != nil || actorRoleID != rolePrimaryAdmin {
				fail++
				continue
			}
		}
	...
	response.WriteJSON(w, response.OK(map[string]int{"successCount": success, "failCount": fail}))
}
...
func (h *Handler) userBatchResetFlow(w http.ResponseWriter, r *http.Request) {
	...
	now := time.Now().UnixMilli()
	for _, id := range req.IDs {
		h.repo.ResetUserFlowByUser(id, now)
	}
	response.WriteJSON(w, response.OK(map[string]int{"successCount": len(req.IDs), "failCount": 0}))
}
...
func (h *Handler) userUpdateOrder(w http.ResponseWriter, r *http.Request) {
	...
	for _, item := range req.Users {
		_ = h.repo.UpdateUserOrder(item.ID, item.Inx, now)
	}
	response.WriteJSON(w, response.OKEmpty())
}
...
func (h *Handler) nodeInstall(w http.ResponseWriter, r *http.Request) {
	...
	cmd := fmt.Sprintf("curl -L https://gcode.hostcentral.cc/https://github.com/shihuan8/tk-beta/releases/download/%s/install.sh -o ./install.sh && chmod +x ./install.sh && VERSION=%s ./install.sh -a %s -s %s", version, version, processServerAddress(panelAddr), secret)
...
func (h *Handler) userTunnelBatchUpdateStatus(w http.ResponseWriter, r *http.Request) {
	...
	if err := decodeJSON(r.Body, &req); err != nil || len(req.IDs) == 0 || (req.Status != 0 && req.Status != 1) {
		response.WriteJSON(w, response.ErrDefault("请求参数错误"))
		return
	}
	success, fail := 0, 0
	for _, id := range req.IDs {
		userID, tunnelID, err := h.repo.GetUserTunnelUserAndTunnel(id)
		if err != nil {
			fail++
			continue
		}
		_, flow, numRaw, expTime, flowReset, speedID, _, err := h.repo.GetExistingUserTunnel(userID, tunnelID)
		if err != nil {
			fail++
			continue
		}
		if err := h.repo.UpdateUserTunnel(id, flow, int(numRaw), expTime, flowReset, nullableNullInt64(speedID), req.Status); err != nil {
			fail++
		} else {
			success++
		}
	}
	response.WriteJSON(w, response.OK(map[string]int{"successCount": success, "failedCount": fail}))
}

func (h *Handler) userTunnelAssignPackageGroup(w http.ResponseWriter, r *http.Request) {
	var req struct {
		UserID      int64 `json:"userId"`
		UserGroupID int64 `json:"userGroupId"`
	}
	if err := decodeJSON(r.Body, &req); err != nil || req.UserID <= 0 || req.UserGroupID <= 0 {
		response.WriteJSON(w, response.ErrDefault("请求参数错误"))
		return
	}
	group, err := h.repo.GetUserGroup(req.UserGroupID)
	if err != nil {
		response.WriteJSON(w, response.Err(-2, err.Error()))
		return
	}
	if group == nil || group.Status != 1 {
		response.WriteJSON(w, response.ErrDefault("套餐分组不可用"))
		return
	}
	now := time.Now().UnixMilli()
	if _, err := h.repo.ReplaceUserGroupsByUserID(req.UserID, []int64{req.UserGroupID}, now); err != nil {
		response.WriteJSON(w, response.Err(-2, err.Error()))
		return
	}
	_ = h.syncPermissionsByUserGroup(req.UserGroupID)
	response.WriteJSON(w, response.OKEmpty())
}
...
func (h *Handler) forwardUpdate(w http.ResponseWriter, r *http.Request) {
	...
	if !isAdminRoleID(actorRole) && hasSpeedID && requestedSpeedID != nil && !sameSpeedLimitSelection(forward.SpeedID, requestedSpeedID) {
...
func (h *Handler) forwardResume(w http.ResponseWriter, r *http.Request) {
	...
	_ = h.controlForwardServices(forward, "DeleteService", true)
...
func (h *Handler) speedLimitCreate(w http.ResponseWriter, r *http.Request) {
	...
	speed := normalizeSpeedLimitBPS(asInt(req["speed"], 100))
...
func (h *Handler) groupCreate(w http.ResponseWriter, r *http.Request, table string) {
	...
	now := time.Now().UnixMilli()
	if err := h.repo.GroupCreateWithPackage(table, name, asInt(req["status"], 1), asInt64(req["flowLimit"], 0), asInt64(req["speedLimit"], 0), asInt(req["ruleQuota"], 0), asInt(req["durationDays"], 0), now); err != nil {
		response.WriteJSON(w, response.Err(-2, err.Error()))
		return
	}
	response.WriteJSON(w, response.OKEmpty())
...
func (h *Handler) groupUpdate(w http.ResponseWriter, r *http.Request, table string) {
	...
	if err := h.repo.GroupUpdateWithPackage(table, id, asString(req["name"]), asInt(req["status"], 1), asInt64(req["flowLimit"], 0), asInt64(req["speedLimit"], 0), asInt(req["ruleQuota"], 0), asInt(req["durationDays"], 0), time.Now().UnixMilli()); err != nil {
		response.WriteJSON(w, response.Err(-2, err.Error()))
		return
	}
	response.WriteJSON(w, response.OKEmpty())
...
func normalizeSpeedLimitBPS(speed int) int {
