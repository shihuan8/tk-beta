...
func (h *Handler) ensureForwardAccessByActor(actorUserID int64, actorRole int, forwardID int64) (*forwardRecord, error) {
	...
	if !isAdminRoleID(actorRole) && forward.UserID != actorUserID {
...
func (h *Handler) syncForwardServicesWithWarnings(forward *forwardRecord, method string, allowFallbackAdd bool) ([]string, error) {
	...
	for _, fp := range ports {
		...
		if err != nil && strings.EqualFold(strings.TrimSpace(method), "UpdateService") && isAddressAlreadyInUseError(err) {
			staleServiceName := serviceNameFromCreateServiceError(err.Error())
			if cleanupErr := h.deleteServiceNameFromAddressInUseError(node.ID, err); cleanupErr != nil && !isNotFoundError(cleanupErr) {
				return warnings, fmt.Errorf("节点 %s 清理占用旧服务失败: %w", node.Name, cleanupErr)
			}
			time.Sleep(300 * time.Millisecond)
			err = h.rebindForwardServiceOnSelfOccupiedPort(forward, node, fp.Port, services, staleServiceName)
		}
...
func (h *Handler) rebindForwardServiceOnSelfOccupiedPort(forward *forwardRecord, node *nodeRecord, port int, services []map[string]interface{}, extraServiceName string) error {
	...
	if extraServiceName != "" {
		extraBase := strings.TrimSuffix(strings.TrimSuffix(strings.TrimSpace(extraServiceName), "_tcp"), "_udp")
		if extraBase != "" {
			bases = append([]string{extraBase}, bases...)
		}
	}
...
func (h *Handler) deleteServiceNameFromAddressInUseError(nodeID int64, err error) error {
	if h == nil || nodeID <= 0 || err == nil {
		return nil
	}
	name := serviceNameFromCreateServiceError(err.Error())
	if name == "" {
		return nil
	}
	...
	if base != "" && base != name {
		services = append(services, base+"_tcp", base+"_udp", base)
	}
	_, deleteErr := h.sendNodeCommand(nodeID, "DeleteService", map[string]interface{}{"services": uniqueStrings(services)}, false, true)
	return deleteErr
}

func serviceNameFromCreateServiceError(message string) string {
	message = strings.TrimSpace(message)
	if message == "" {
		return ""
	}
	matches := regexp.MustCompile(`(?i)create\s+service\s+([^\s]+)\s+failed`).FindStringSubmatch(message)
	if len(matches) < 2 {
		return ""
	}
